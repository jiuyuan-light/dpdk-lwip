...
static int launch_one_lcore(__rte_unused void *dummy)
{
	...
	const uint64_t drain_tsc = (rte_get_tsc_hz() + US_PER_S - 1) / US_PER_S *
			BURST_TX_DRAIN_US;
	struct rte_eth_dev_tx_buffer *buffer;
    struct netif *nif = (struct netif *)&g_netif;
...
dpdk_init(int argc, char **argv)
{
    ...
    /* TODO, now use 0 */
    g_port_info.port = 0;
    g_port_info.pktmbuf_pool = l2fwd_pktmbuf_pool;
	rte_eth_macaddr_get(0, &g_port_info.src_addr);
	printf("Port %u MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8
			   " %02" PRIx8 " %02" PRIx8 " %02" PRIx8 "\n",
			(unsigned)0,
			g_port_info.src_addr.addr_bytes[0], g_port_info.src_addr.addr_bytes[1],
			g_port_info.src_addr.addr_bytes[2], g_port_info.src_addr.addr_bytes[3],
			g_port_info.src_addr.addr_bytes[4], g_port_info.src_addr.addr_bytes[5]);
